...
export class driver {
    ...
    @ManyToOne(() => driver_situation, (driver_situation) => driver_situation.drivers, { cascade: true})
    @JoinColumn({ name: 'driver_situation_id' })
    driver_situation: driver_situation;

    @ManyToOne(() => driver_category, (driver_category) => driver_category.drivers, { cascade: true})
